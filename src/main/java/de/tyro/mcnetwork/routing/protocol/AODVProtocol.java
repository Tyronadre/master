...
public class AODVProtocol implements RoutingProtocol {
    ...
    private final HashMap<IP, RouteDiscoveryState> discoveries = new HashMap<>();
    private final Map<Long, List<Runnable>> tickActions = new HashMap<>();
    ...
    public void discoverRoute(IP destIp) {
        ...
        addActionOnTick(now + (int) Math.pow(2, state.rreqAttempts) * NET_TRAVERSAL_TIME, () -> discoverRoute(destIp));
        state.rreqAttempts++;
    ...
    private void addActionOnTick(long tick, Runnable action) {
        tickActions.computeIfAbsent(tick, l -> new ArrayList<>()).add(action);
    }
    ...
    private void handleRREP(AODVRREPPacket rrep) {
        ...
        var destinationEntry = routingTable.get(rrep.destinationIP);
        // Then the forward route for this destination is created if it does not already exist.
        if (destinationEntry == null) {
            destinationEntry = new RouteEntry(rrep.destinationIP);
            routingTable.put(rrep.destinationIP, destinationEntry);
        }
        ...
        if (!destinationEntry.seqValid ||
                rrep.destSeqNumber > destinationEntry.seqNumber ||
                (rrep.destSeqNumber == destinationEntry.seqNumber && (!destinationEntry.valid || rrep.hopCount < destinationEntry.hopCount))
        ) {
        ...
        if (node.getIP().equals(rrep.originatorIP)) {
            discoveries.remove(rrep.originatorIP);
            var pending = pendingData.remove(rrep.destinationIP);
            if (pending != null) pending.forEach(this::unicast);
            return;
        }
        ...
        var reverseEntry = routingTable.get(rrep.originatorIP);
        ...
        simulator.sendUnicast(node, simulator.getNodeByIp(nextHopToOriginator), rrep, rrep.getNetworkFrame().ttl - 1);
    }
    ...
    private void generateRREP(AODVRREQPacket rreq) {
        ...
        var destinationEntry = routingTable.get(rreq.destinationIP);
        var originatorEntry = routingTable.get(rreq.originatorIP);
        boolean isDestination = rreq.destinationIP.equals(node.getIP());
        boolean isIntermediate =
                !isDestination &&
                        destinationEntry != null &&
                        destinationEntry.valid &&
                        destinationEntry.seqValid &&
                        destinationEntry.seqNumber >= rreq.destinationSequenceNumber &&
                        !rreq.destinationOnlyFlag;

        if (!isDestination && !isIntermediate) return;

        AODVRREPPacket rrep = new AODVRREPPacket(rreq.originatorIP, rreq.destinationIP, false, false, 0, 0, 0);
        ...
        // the originator.
        simulator.sendUnicast(node, simulator.getNodeByIp(originatorEntry.nextHop), rrep, rreq.hopCount * 2);
        ...
        if (isIntermediate && rreq.gratuitousFlag) {
            var gRrep = new AODVRREPPacket(rreq.destinationIP, rreq.originatorIP, false, false, originatorEntry.hopCount, rreq.originatorSequenceNumber, originatorEntry.lifetime);
            simulator.sendUnicast(node, simulator.getNodeByIp(destinationEntry.nextHop), gRrep, 100);
        }
    }
    ...
    private void handleRREQ(AODVRREQPacket rreq) {
        ...
        String key = key(rreq.originatorIP, rreq.rreqId);
        ...
        var originator = rreq.originatorIP;
        ...
        long minimalLifetime =
                simulator.getSimTime()
                        + 2L * NET_TRAVERSAL_TIME
                        - 2L * rreq.hopCount * NODE_TRAVERSAL_TIME;
        ...
        RouteEntry destRoute = routingTable.get(rreq.destinationIP);

        // Case 1: this node is the destination
        if (node.getIP().equals(rreq.destinationIP)) {
            generateRREP = true;
        }

        // Case 2: intermediate node with a fresh enough route
        else if (!rreq.destinationOnlyFlag && destRoute != null && destRoute.valid) {
            if (!rreq.unknownSeqFlag &&
                    destRoute.seqValid &&
                    destRoute.seqNumber >= rreq.destinationSequenceNumber) {
                generateRREP = true;
            }
        }
    ...
    @Override
    public void unicast(INetworkPacket packet) {
        ...
        if (hasRoute(packet.getDestinationIP())) {
            ...
            if (simulator.sendUnicast(node, simulator.getNodeByIp(entry.nextHop), packet, 100)) {
    ...
    public void tick() {
        ...
        if (tickActions.containsKey(now)) tickActions.get(now).forEach(Runnable::run);
        tickActions.remove(now);
    ...
    private static class RouteDiscoveryState {
        ...
        public RouteDiscoveryState() {
            nextRreqAttemptsResetInterval = SimulationEngine.getInstance().getSimTime() + 1000;
        }
